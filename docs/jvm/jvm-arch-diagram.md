...
- 本地方法接口(Native Interface): 融合不同开发语言的原始库为java所用
- 内存空间(Run Data Area)： Jvm内存空间结构模型


## Jvm内存空间结构模型
- 线程私有: 程序计数器、虚拟机栈、本地方法栈
- 线程共享: MetaSpace元空间(类加载信息OOM)、常量池(字面量和符号引用OOM)、堆(数组和类对象OOM)
...
递归过深，栈帧数超出了虚拟栈深度，解决这个问题的思路是限制递归的深度或是使用循环替代递归方法。

## 虚拟机栈过多会引发`java.lang.OutOfMemoryError`异常?

## 元空间(MetaSpace)与永久代(PermGen)的区别?
- 元空间使用本地内存，而永久代使用的是jvm的内存,元空间的本质和永久代类似，都是对JVM规范中方法区的实现。不过元空间与永久代之间最大的区别在于：**元空间并不在虚拟机中，而是使用本地内存**


java8以后不会再出现 `java.lang.OutOfMemoryError: PermGen space`异常

## MetaSpace相比PermGen的优势
- 字符串常量存在永久代中，容易出现性能问题和内存溢出
- 类和方法的信息大小难以确定，给永久代的大小指定带来款南
- 永久代会为GC带来不必要的复杂性
- 方法HotSpot与其他JVM如Jrockit的集成


## 什么是堆(Heaps)
- Heap是jvm所管理内存中最大的一块，在jvm初始化时创建，其创建的唯一目的是管理java对象的实例。**对象实例的分配区域**
- GC管理的主要区域(新生代、老年代)
